from ...algorithm import Exploration

import copy
import numpy as np

class BanditRunner(Exploration):
    '''
    A bandit runner
    '''
    #--------------------------------------------------------------------------------------
    ...
    def reset(self):
        ...
        self.orderChoices = []
        # Train once
        self.uncertaintyModel.train(trainMask)
        ...
        for userIndex in range(exploreMask.shape[0]):
            ...
            for stateCounter in range(int(maxStateCounter[int(userIndex)])):
                if stateCounter >= tempMaxNumItem:
                    break
                currUncertaintyModel.load(self.fileLocation + self.modelName + self.fileExt)
                # currUncertaintyModel.load(self.fileName), load from scratch for each item
                currUncertaintyModel.train(trainMask)
